...
- Markdown、docstringなどの文章は、固定文字数や行の長さだけを理由に改行しない。
- 文、段落、箇条書きなど、意味が完結する区切りで改行する。
- 専門用語、固有名詞、修飾関係が続く語句は、行の長さをそろえるために途中で分割しない。
- 行長を調整する必要がある場合は、語句を分割せずに文または段落を再構成する。
